...
/**
 * Props for the WorkoutCardScalable component
 */
export interface WorkoutCardScalableProps {
 ...
 * workout progress for a single person with fluid scaling based on viewport size.
 *
 * Features:
 ...
 * - Viewport-based responsive scaling for fullscreen displays
 */
export const WorkoutCardScalable = ({
  personName,
  exercise,
  restTimeRemaining,
  variant = "active",
  onSetComplete,
}: WorkoutCardScalableProps) => {
  ...
  const getCardClasses = () => {
    const baseClasses =
      "relative flex flex-col gap-fluid-4 p-fluid-6 rounded-fluid-2xl shadow-lg hover:shadow-xl transition-all duration-300 border";
  ...
  return (
    <div className={getCardClasses()}>
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-fluid-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent dark:from-indigo-400 dark:to-purple-400">
          {personName}
        </h3>
      </div>
      ...
      <div className="flex gap-1">
        <h4 className="text-fluid-5xl font-semibold text-gray-700 dark:text-gray-200">
          {currentSet.weight}&nbsp;
          <span className="text-fluid-2xl text-gray-500 dark:text-gray-400">
            {exercise.weightUnit}
          </span>
        </h4>
      </div>
      ...
      <div className="flex flex-col gap-fluid-2">
        <div className="inline-flex items-center gap-1.5 flex-wrap">
          {exercise.sets.map((set, index) => (
            <button
              ...
              className={`text-fluid-2xl transition-transform duration-200 ${
                set.completed
        ...
        <div className="text-fluid-sm text-gray-600 dark:text-gray-400">
      ...
      {nextSet && !nextSet.completed && (
        <div className="flex items-center gap-fluid-2 px-fluid-3 py-fluid-1 rounded-fluid-lg bg-indigo-50 dark:bg-indigo-900/30 w-fit">
          <svg
            className="w-fluid-4 h-fluid-4 text-indigo-600 dark:text-indigo-400"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <span className="text-fluid-sm font-medium text-indigo-600 dark:text-indigo-400">
